...
const login = async (req, res) => {
    try {
        ...
        const user = await User.findOne({ username, deleted: false }).populate({ path: 'roles' });
        ...
        res.status(500).json({ error: error?.message || 'A occurred' }); // send actual error for debugging
    }
};
